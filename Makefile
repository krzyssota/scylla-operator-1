...
.PHONY: helm-release
helm-release:
	gsutil rsync -d $(HELM_BUCKET) helm/repo
	helm package helm/scylla-operator helm/scylla helm/scylla-manager -d helm/repo
	helm repo index helm/repo --url $(HELM_REPOSITORY) --merge helm/repo/index.yaml
	gsutil rsync -d helm/repo $(HELM_BUCKET)
